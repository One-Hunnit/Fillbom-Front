...
const UploadProfileImage = ({ currentImageUrl, onChange }: IUploadProfileImageProps) => {
  const uploadImage = (getter: () => Promise<string | undefined>) => async () => {
    const base64 = await getter();
    if (!base64) return;
    const res = await client.POST('/image/upload', { body: { image: base64 } });
    console.log(res);
    if (res?.data?.data) {
  ...
  return (
    <View style={uploadProfileImageStyles.container}>
      ...
      <View style={uploadProfileImageStyles.buttonContainer}>
        <MonoButton
          onPress={uploadImage(pickImageFromCamera)}
          buttonStyle={uploadProfileImageStyles.button}
          icon={IconCamera}
          text="사진 촬영"
        />
        <MonoButton
          onPress={uploadImage(pickImageFromLibrary)}
          buttonStyle={uploadProfileImageStyles.button}
          icon={IconPhoto}
          text="앨범 선택"
        />
